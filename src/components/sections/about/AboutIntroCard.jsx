...
function AboutIntroCard({ title, body, badges, direction = "left" }) {
  ...
  return (
    ...
    >
      <h3 className="mb-4 bg-linear-to-r from-blue-400 to-cyan-400 bg-clip-text text-2xl font-semibold text-transparent">
        {title}
      </h3>
      ...
      <div className="mt-6 flex flex-wrap gap-3">
        {badges.map((badge) => (
          <span
            key={badge}
            className="rounded-full border border-blue-500/20 bg-linear-to-r from-blue-500/15 to-cyan-500/15 px-4 py-1.5 text-sm font-medium text-blue-300 backdrop-blur-sm"
          >
            {badge}
          </span>
        ))}
